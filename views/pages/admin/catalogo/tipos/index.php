...
<section class="mb-4">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
        <div>
            ...
            <h1 class="h3 mb-0">Tipos de partes</h1>
        </div>
    </div>
</section>
        ...
        <div class="card h-100">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <?php if ($editing) : ?>
                        <a class="btn btn-sm btn-outline-secondary" href="<?= url('/configuracion/tipos-partes') ?>">
                            Cancelar
                        </a>
                    <?php endif; ?>
                </div>
                ...
                <?php endif; ?>
                <form method="post" action="<?= $editing ? url('/configuracion/tipos-partes/' . (int) $editing['id']) : url('/configuracion/tipos-partes') ?>" class="vstack gap-3">
                    <?php if ($editing) : ?>
                    ...
                    <div class="form-check form-switch mb-2">
                        <input class="form-check-input" type="checkbox" name="requiere_stock" id="requiere-stock" <?= (int) $oldValue('requiere_stock', 1) === 1 ? 'checked' : '' ?>>
                        <label class="form-check-label" for="requiere-stock">Requiere stock</label>
                        <div class="form-text small">Si se desmarca, este tipo de parte no validará disponibilidad en planificación.</div>
                    </div>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            <?php foreach ($tipos as $tipo) : ?>
                                <tr>
                                    <td class="fw-semibold"><?= View::escape($tipo['codigo']) ?></td>
                                    ...
                                    <td class="text-center">
                                        <?php if ((bool) ($tipo['requiere_stock'] ?? true)) : ?>
                                            <i class="fa-solid fa-check text-success" title="Controla Stock"></i>
                                        <?php else : ?>
                                            <i class="fa-solid fa-minus text-muted" title="Sin control de stock"></i>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-end">
                                        <div class="btn-group btn-group-sm">
                                            <a class="btn btn-outline-secondary" href="<?= url('/configuracion/tipos-partes/' . (int) $tipo['id'] . '/editar') ?>">
                                                <i class="fa-solid fa-pen"></i>
                                            </a>
                                            <form method="post" action="<?= url('/configuracion/tipos-partes/' . (int) $tipo['id']) ?>" onsubmit="return confirm('¿Eliminar tipo de parte?');">
