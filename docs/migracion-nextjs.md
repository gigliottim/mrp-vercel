...
**Estado P7 (2026-09-06):** **Import/Export CSV migrado** (88 tests API). Endpoints nuevos: plantilla/export/import de Partes+Variantes (28 columnas, upsert por código, reporte por fila), plantilla/export/import de Maestro BOM (detección de ciclos por BFS sobre grafo en memoria, duplicados → skip), recálculo de geometría (superficie/volumen con conversión de unidades) y verificación de borrado seguro de variante (compras/movimientos/BOM padre-hijo). Frontend: páginas `/productos/partes/importar` y `/productos/maestro/importar` con upload + reporte visual. **Hallazgos P7**: PostgREST no acepta newlines en select (PGRST100); embed inverso requiere hint en la tabla que tiene la FK; `creado_por` es bigint legacy (no UUID); `factor_conversion` SÍ existe en partes. **Pendiente**: Agente AI (chat `/agent`) y passwords temporales (mantenidas por decisión en desarrollo).
