...
**Estado P6 (2026-09-06):** **Menú completo: 35/35 rutas HTTP 200 en producción.** Se implementaron las 15 pantallas faltantes: Maestro de Productos (árbol BOM), Copiar Componentes, Reemplazar Partes en el Maestro, Sugerencias MRP (fabricabilidad por BOM), Stock Crítico, Órdenes Planificadas, 4 reportes (Destino de Partes, Listado de Ingeniería, Planificación de Producción, Resumen por Grupos), Empresa/Usuarios/Roles/Permisos (ACL por rol), Vista Gantt, Dashboard de Operaciones y Panel inicial. **API ampliada** (81 tests): 9 endpoints nuevos — BOM avanzado (tree/where-used/copiar/reemplazar vía 3 RPCs `bom_tree`/`bom_where_used`/`bom_validate_add`), sugerencias, inventario crítico, empresa/usuarios/roles/permisos, reportes, operaciones/gantt. **Hallazgos**: joins PostgREST ambiguos requieren hints de FK (`!tabla_columna_fkey`); tabla `roles` es estilo spatie (name/guard_name); `createAdminClient()` es solo REST (auth admin vía fetch); `factor_conversion` no existe en BD migrada. **Pendiente**: cambiar passwords temporales de los 3 usuarios.
