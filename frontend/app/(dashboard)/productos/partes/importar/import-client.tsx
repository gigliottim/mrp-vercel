...
type Report = {
  ...
  created_parts: number; updated_parts: number;
  created_variants: number; updated_variants: number;
...
export function ImportClient({ token }: { token: string }) {
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await fetch('/api/v1/import-export/partes/import', {
        method: 'POST',
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-4">
        ...
        <a
          href={`/api/v1/import-export/partes/template`}
          className="text-sm text-primary underline"
          onClick={(e) => {
            e.preventDefault()
            fetch('/api/v1/import-export/partes/template', { headers: { Authorization: `Bearer ${token}` } })
              .then((r) => r.blob())
              .then((b) => {
                const url = URL.createObjectURL(b)
                const a = document.createElement('a')
                a.href = url
                a.download = 'plantilla_import_partes_variantes.csv'
                a.click()
        ...
        <a
          ...
          onClick={(e) => {
            e.preventDefault()
            fetch('/api/v1/import-export/partes/export', { headers: { Authorization: `Bearer ${token}` } })
              .then((r) => r.blob())
              .then((b) => {
                const url = URL.createObjectURL(b)
                const a = document.createElement('a')
                a.href = url
                a.download = 'export_partes_variantes.csv'
                a.click()
                URL.revokeObjectURL(url)
              })
          }}
      ...
      {report ? (
        <div className="space-y-4">
          <div className="grid gap-2 sm:grid-cols-5">
            ...
            <div className="rounded-md border p-3"><p className="text-xs text-muted-foreground">Partes creadas</p><p className="text-xl font-bold">{report.created_parts}</p></div>
            <div className="rounded-md border p-3"><p className="text-xs text-muted-foreground">Variantes creadas</p><p className="text-xl font-bold">{report.created_variants}</p></div>
          </div>
