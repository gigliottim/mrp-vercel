...
**Estado P8 (2026-09-06):** **Agente AI migrado** (93 tests API). Chat guiado paso a paso en `/agent`: flujos deterministas para crear Parte+Variantes (11 campos + bucle de variantes), Entidades (6 campos) y BOM (padre + componentes × N), con lookups por chips (tipos, grupos, unidades, estados, tipos de entidad), confirmación con guardado real en BD y cancelación. `general_query` usa Ollama Cloud si hay `AGENT_AI_API_KEY` (modo offline sin key). Estado de conversación en memoria con TTL 30 min (serverless). **Fix CORS**: la API no tenía middleware CORS — el chat del browser fallaba con "Failed to fetch"; habilitado `hono/cors` con allowlist (mimrp.com.ar, alias Vercel, localhost). **Pendiente**: passwords temporales (mantenidas por decisión en desarrollo).
